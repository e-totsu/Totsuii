...
function clone(obj) {
    ...
    for (const key in obj) if (obj[has](key)) {
...
var toHex = function (color) {
    const i = glob.doc.getElementsByTagName("head")[0] || glob.doc.getElementsByTagName("svg")[0],
        red = "rgb(255, 0, 0)";
...
function getSomeDefs(el) {
    let p = el.node.ownerSVGElement && wrap(el.node.ownerSVGElement) ||
...
setInterval(function () {
    for (const key in hub) if (hub[has](key)) {
        const el = hub[key],
            node = el.node;
...
function Element(el) {
    ...
    if (this.type in {g: 1, mask: 1, pattern: 1, symbol: 1}) {
        for (const method in Paper.prototype) if (Paper.prototype[has](method)) {
...
function Paper(w, h) {
    ...
    if (w && w.tagName && w.tagName.toLowerCase() == "svg") {
        ...
        res.defs = defs;
        for (const key in proto) if (proto[has](key)) {
            res[key] = proto[key];
        }
...
Snap.ajax = function (url, postData, callback, scope){
    ...
    if (req) {
        if (is(postData, "function")) {
            ...
            const pd = [];
            for (const key in postData) if (postData.hasOwnProperty(key)) {
    ...
    elproto.data = function (key, value) {
        ...
        if (arguments.length == 1) {
            if (Snap.is(key, "object")) {
                for (const i in key) if (key[has](i)) {
                    this.data(i, key[i]);
                }
                return this;
            }
...
Snap.plugin(function (Snap, Element, Paper, glob, Fragment) {
    ...
    elproto.inAnim = function () {
        const el = this,
            res = [];
        for (const id in el.anims) if (el.anims[has](id)) {
            (function (a) {
                res.push({
    ...
    elproto.animate = function (attrs, ms, easing, callback) {
        ...
        let fkeys = [], tkeys = [], keys = {}, from, to, f, eq,
            el = this;
        for (const key in attrs) if (attrs[has](key)) {
            if (el.equal) {
        ...
        const now = mina.time(),
            anim = mina(fkeys, tkeys, now, now + ms, mina.time, function (val) {
                const attr = {};
                for (const key in keys) if (keys[has](key)) {
...
Snap.plugin(function (Snap, Element, Paper, glob, Fragment) {
    ...
    (function (matrixproto) {
        ...
        matrixproto.skew = function (x, y) {
            ...
            x = Snap.rad(x);
            y = Snap.rad(y);
            const c = math.tan(x).toFixed(9);
            const b = math.tan(y).toFixed(9);
        ...
        matrixproto.split = function () {
            ...
            // scale and shear
            const row = [[this.a, this.b], [this.c, this.d]];
...
Snap.plugin(function (Snap, Element, Paper, glob, Fragment) {
    ...
    eve.on("snap.util.grad.parse", function parseGrad(string) {
        ...
        len--;
        for (let i = 0; i < len; i++) if ("offset" in stops[i]) {
    ...
    (function () {
        ...
        function GaddStop(color, offset) {
            ...
            for (let i = 0; i < stops.length; i++) {
                const stopOffset = parseFloat(stops[i].attr("offset"));
        ...
        (function (eventName) {
            Snap[eventName] = elproto[eventName] = function (fn, scope) {
                if (Snap.is(fn, "function")) {
                    ...
                    for (let i = 0, ii = this.events.length; i < ii; i++) if (this.events[i].name == eventName) {
                        try {
